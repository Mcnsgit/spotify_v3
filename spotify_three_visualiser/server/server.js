...
app.put('/me/player', verifyAccessToken, async (req, res) => {
  try {
    const data = await spotifyApiRequest('/me/player', 'put', req.body, req.accessToken);
    res.json(data);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
app.get('/me/player/play', verifyAccessToken, async (req, res) => {
  try { 
    const data = await spotifyApiRequest('/me/player/play', 'put', {}, req.accessToken);
    res.json(data);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
app.get('/auth/search', verifyAccessToken, async (req, res) => {
  try {
    const data = await spotifyApiRequest('/search', 'get', { q: req.query.track, type: 'track' }, req.accessToken);
    res.json(data);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
